...
//    List<MagicCard> findByRarityAndMagicTypesListAndColorIdentityList(String rarity, List<String> magicTypesList, List<String> colorIdentityList);
    ...
    @Query(value = "SELECT distinct a.* FROM magic_card a \n" +
                    " INNER JOIN magic_card_types c on a.magic_card_id = c.magic_card_MagicCardId " +
                    " INNER JOIN magic_card_legalities d on a.magic_card_id = d.magic_card_MagicCardId " +
                    " INNER JOIN legalities e on d.legalities_LegalitiesId = e.legalities_id " +
                    "WHERE a.magic_card_id Not IN(" +
                    "SELECT DISTINCT a.magic_card_id from magic_card a \n" +
                    "INNER JOIN magic_card_color_identity b on a.magic_card_id = b.magic_card_MagicCardId " +
                    "WHERE b.color_identity_color IN(:C1,:C2,:C3,:C4,:C5)) " +
                    "AND c.types_typeName = :cardType " +
                    "AND a.rarity = :rarity " +
                    "AND e.format = :format " +
